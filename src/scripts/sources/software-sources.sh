...
## Avisos do APT
#printf "%s" "/usr/bin/apt \"\$@\" 2>&1 | grep -v 'Policy will reject signature within a year'" | sudo tee /usr/local/bin/apt >/dev/null
#sudo chmod +x /usr/local/bin/apt
#printf 'Apt::Cmd::Disable-Script-Warning true;' | sudo tee /etc/apt/apt.conf.d/warnings >/dev/null
