...
if [ -f /usr/share/applications/$LN.desktop ] || [ -f /usr/local/share/applications/$LN.desktop ];then
    if [ -f  /usr/share/applications/$LN.desktop ];then
        if ! grep 'StartupWMClass=$SWMC' /usr/share/applications/$LN.desktop>/dev/null;then
            sed -i '/^\[Desktop Entry\]/a StartupWMClass=$SWMC' /usr/share/applications/$LN.desktop
        fi
        else
        if [ -f  /usr/local/share/applications/$LN.desktop ];then
            if ! grep 'StartupWMClass=$SWMC' /usr/local/share/applications/$LN.desktop>/dev/null;then
                sed -i '/^\[Desktop Entry\]/a StartupWMClass=$SWMC' /usr/local/share/applications/$LN.desktop
            fi
        fi
    fi
fi
...
local_papirus_icon() {
    if [ "$(gsettings get org.gnome.desktop.interface icon-theme)" == "'Papirus-Dark'" ]; then
        ...
        ln -fs /usr/share/icons/Papirus-Dark/128x128/apps/"$ICON_ORIG".svg "$HOME"/.icons/Papirus-Dark/128x128/apps/"$ICON_REPL".svg
    fi
}